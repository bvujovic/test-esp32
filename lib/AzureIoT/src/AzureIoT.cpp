...
void azure_iot_init(azure_iot_t *azure_iot, azure_iot_config_t *azure_iot_config)
{
    ...
    if (az_span_is_content_equal(azure_iot_config->device_key, AZ_SPAN_EMPTY) && (az_span_is_content_equal(azure_iot_config->device_certificate, AZ_SPAN_EMPTY) || az_span_is_content_equal(
...
void azure_iot_do_work(azure_iot_t *azure_iot)
{
    ...
    case azure_iot_state_started:
        ...
        if (result != 0 || azure_iot->config->mqtt_client_interface.mqtt_client_init(
                               &mqtt_client_config, &azure_iot->mqtt_client_handle) != 0)
        {
            azure_iot->state = azure_iot_state_error;
    ...
    case azure_iot_state_provisioned:
        ...
        if (azure_iot->config->use_device_provisioning && azure_iot->mqtt_client_handle != NULL && azure_iot->config->mqtt_client_interface.mqtt_client_deinit(azure_iot->mqtt_client_handle) != 0)
        {
            azure_iot->state = azure_iot_state_error;
    ...
    case azure_iot_state_ready:
        ...
        else if ((azure_iot->sas_token_expiration_time - now) < SAS_TOKEN_REFRESH_THRESHOLD_IN_SECS)
        {
            ...
            if (azure_iot->config->mqtt_client_interface.mqtt_client_deinit(
                    azure_iot->mqtt_client_handle) != 0)
            {
                azure_iot->state = azure_iot_state_error;
                LogError("Failed de-initializing MQTT client.");
                return;
